...
public class TP03Q05 {
    ...
    static void quicksort(Personagens[] p, String[] arr, int left, int right) {
        int i = left, j = right;
        ...
        if (left < j) {
            quicksort(p, arr, left, j);
        }
        if (i < right) {
            quicksort(p, arr, i, right);
        }
    }

    static void quicksort(Personagens[] p, String[] arr, int left, int right, int k) {
        int i = left, j = right;
        String pivot = arr[(left + right) / 2];
        while (i <= j) {
            while (arr[i].compareTo(pivot) < 0) {
        ...
        if (left < j) {
            quicksort(p, arr, left, j, k);
        }
        if (i < right && i < k) {
            quicksort(p, arr, i, right, k);
        }
    }
    ...
    static void sort(Personagens[] p, String[] att, int start, int length, int k) {
        ...
        quicksort(p, arr, start, length - 1, k);
    }
    ...
    static void doOp(String text, Lista<Personagens> list, Personagens[] arr) throws Exception {
        String op = "";
        int i = 0;
        ...
        switch (op) {
            case "II":
                list.inserirInicio(find(text, arr));
                break;
            case "I*":
                i = 0;
                op = "";
                ...
                list.inserir(find(text, arr), Integer.parseInt(op));
                break;
            case "IF":
                list.inserirFim(find(text, arr));
                break;
            case "RI":
                out.println("(R) " + list.removerInicio().getName());
                break;
            case "R*":
                out.println("(R) " + list.remover(Integer.parseInt(text)).getName());
                break;
            case "RF":
                out.println("(R) " + list.removerFim().getName());
                break;
            default:
                break;
        }
    ...
    public static void main(String[] args) {
        ...
        try {
            ...
            Lista<Personagens> CharList = new Lista<Personagens>();
            while (!buff.equals("FIM")) {
                Personagens p = find(buff, hPCharacters);
                if (p != null) {
                    CharList.inserirFim(p);
                } else {
            ...
            while (0 < amt--) {
                op = in.readLine();
                doOp(op, CharList, hPCharacters);
            }

            int siz = CharList.lenght();
            for (int i = 0; i < siz; i++) {
                out.print("[" + i + separation);
                CharList.removerInicio().printall();
                out.println("]");
            }
