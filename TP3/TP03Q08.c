...
void inserir(Personagens *p, Lista *F)
{
...
void swapLista(No *a, No *b)
{
    Personagens *tmp = b->val;
    b->val = a->val;
    a->val = tmp;
}

// ----------------- QUICKSORT -----------------
void quicksort(int left, int right, No *s, No *e, char **arr)
{
    int i = left, j = right;
    No *ts = s, *te = e;
    ...
    if (left < j)
    {
        quicksort(left, j, s, te, arr);
    }
    if (i < right)
    {
        quicksort(i, right, ts, e, arr);
    }
}
// -------------------------------------------------------
...
{
    No *tmpNo = f->start;
    char **arr = (char **)malloc(f->length * sizeof(char *));
    ...
    for (size_t i = 0; i < f->length; i++)
    {
        ...
        memset(tmp, '\0', 500);
        strcat(tmp, strdup(tmpNo->val->house));
        strcat(tmp, " ");
        strcat(tmp, strdup(tmpNo->val->name));
        tmpNo = tmpNo->next;

        arr[i] = strdup(tmp);
    }

    quicksort(0, f->length - 1, f->start, f->end, arr);
}
