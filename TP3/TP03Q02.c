...
void insert(Personagens p, int pos)
{
...
void insertTop(Personagens p)
{
    insert(p, 0);
}
void insertBottom(Personagens p)
{
    P.p[P.length] = p;
    P.length++;
}

Personagens removeList(int pos)
{
    Personagens ret = P.p[pos];
    P.length--;
...
Personagens removeTop()
{
    return removeList(0);
}

Personagens removeBottom()
{
    Personagens ret = P.p[--P.length];
    return ret;
}
...
void doOp(char *text, Personagens p[404])
{
    ...
    else if (strcmp(op, "I*") == 0)
    {
        ...
        insert(find(p, text), atoi(op));
    }
    else if (strcmp(op, "IF") == 0)
    {
        insertBottom(find(p, text));
    }
