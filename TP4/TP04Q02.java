...
public class TP04Q02 {
    ...
    static String reflectionFieldToString(String field, Personagens p) {
        String[] fields = new String[1];
        fields[0] = field;
        return reflectionFieldToString(fields, p);
    }
    ...
    static public class ABB<T extends Comparable<T>> implements Comparable<ABB<T>> {
        ...
        public int compareTo(ABB<T> av2) {
            return this.value.compareTo(av2.value);
        }
        ...
        ABNode<T> getNdS(T val) {
            return getNdS(val, start);
        }

        protected ABNode<T> getNdS(T val, ABNode<T> nd) {
            if (nd == null) {
                return nd;
            }
            int comp = val.compareTo(nd.value);
            if (comp > 0) {
                return getNdS(val, nd.right);
            }
            if (comp < 0) {
                return getNdS(val, nd.left);
            }
            return nd;
        }
        ...
        protected ABNode<T> getBiggest(ABNode<T> nd) {
            if (nd == null) {
                return null;
            }
            while (nd.right != null) {
        ...
        ABNode<T> getSmallest() {
            return getSmallest(start);
        }

        protected ABNode<T> getSmallest(ABNode<T> nd) {
            if (nd == null) {
                return null;
            }
            while (nd.left != null) {
    ...
    //
    static void inicializeAbb2(ABB<ABB<Personagens>> a) {
        int order[] = { 7, 3, 11, 1, 5, 9, 13, 0, 2, 4, 6, 8, 10, 12, 14 };

        for (int i = 0; i < order.length; i++) {
            ABB<Personagens> n = new ABB<>();
            n.value = order[i];
    ...
    static void insertAbb2(ABB<ABB<Personagens>> a, Personagens in) {
        ABB<Personagens> n = new ABB<>();
        n.value = in.yearOfBirth % 15;
        ABNode<ABB<Personagens>> tmp = a.getNdS(n);

        if (tmp.value.start == null) {
            tmp.value.start = new ABNode<Personagens>(in);
        } else
            tmp.value.insert(in);
    }

    static boolean SCaminharPre(ABNode<ABB<Personagens>> a, Personagens in) {
        boolean ret = false;
        if (a != null) {

            if (a.value.getNd(in) != null) {
                ret = true;
            }
            if (!ret) {
                System.out.print(" ESQ");
                ret = SCaminharPre(a.left, in);
            }
            if (!ret) {
                System.out.print(" DIR");
                ret = SCaminharPre(a.right, in);
            }
    ...
    public static void main(String[] args) {
        ...
        try {
            ...
            ABB<ABB<Personagens>> CharList = new ABB<ABB<Personagens>>();
            inicializeAbb2(CharList);
            while (!buff.equals("FIM")) {
                Personagens p = find(buff, hPCharacters);
                if (p != null) {
                    insertAbb2(CharList, p);
                } else {
            ...
            while (!buff.equals("FIM")) {
                out.print(buff + " => raiz");
                if (SCaminharPre(CharList.start, new Personagens(buff))) {
                    out.print(" SIM\n");
                } else {
                    out.print(" NAO\n");
                }
