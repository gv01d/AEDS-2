...
No *rotateRight(No *nd)
{
    ...
    // height update
    nd->height = maxi(height(nd->esq), height(nd->dir)) + 1;
    ndL->height = maxi(height(ndL->esq), height(ndL->dir)) + 1;
...
No *rotateLeft(No *nd)
{
    ...
    // height update
    nd->height = maxi(height(nd->esq), height(nd->dir)) + 1;
    ndR->height = maxi(height(ndR->esq), height(ndR->dir)) + 1;

    // new root
    return ndR;
}

int getBalance(No *nd)
{
    if (nd == NULL)
    {
        return 0;
    }
    return height(nd->esq) - height(nd->dir);
}

No *newNd(Personagens *p, char *key)
{
...
No *insert(Personagens *p, char *key, No *nd)
{
    ...
    // normal insert
    int cmp = strcmp(key, nd->key);
    if (cmp < 0)
    {
        nd->esq = insert(p, key, nd->esq);
    }
    else if (cmp > 0)
    {
        nd->dir = insert(p, key, nd->dir);
    }
    else
    {
        return nd;
    ...
    if (bal > 1)
    {
        if (getBalance(nd->esq) < 0)
        {
            nd->esq = rotateLeft(nd->esq);
        }
        return rotateRight(nd);
    }
    if (bal < -1)
    {
        if (getBalance(nd->dir) > 0)
        {
            nd->dir = rotateRight(nd->dir);
        }
...
Personagens *getAvl(char *key, No *nd)
{
    No *tmp = nd;
    if (nd == NULL)
    {
...
char *getKey(Personagens p)
{
    char *tmp = (char *)calloc(500, sizeof(char));
    strcat(tmp, strdup(p.name));
    return tmp;
}
