...
bool search(List l, char *text)
{
    printf("%s", text);

    Node *selected = l.base;
    Personagens *n = (Personagens *)selected->data;

    while (selected != NULL)
    {
        n = (Personagens *)selected->data;

        if (strcmp(n->name, text) == 0)
        {
            return true;
        }
...
Hash *insert(Hash *f, Personagens *p)
{
    if (strcmp(p->name, "Lord Voldemort") == 0)
    {
        printf("W");
    }
    ...
    new.value = p;
    new.prox = NULL;
    int n = hash(p->name, f->size);

    Lista *select = &(f->start[n]);
...
int main()
{
    ...
    Hash *f = (Hash *)malloc(sizeof(Hash));
    f->start = (List *)malloc(21 * sizeof(List));
    for (size_t i = 0; i < 21; i++)
    {
        f->start[i] = *create_list();
    }
    ...
    while (strcmp(r, "FIM") != 0)
    {
        ...
        if (strcmp(r, "efa802c8-ae18-4ae1-a524-49df21d05939") == 0)
        {
            printf("W");
        }

        List *a = f->start + hash(r, 21);
